 ...
 */
@implementation LibreUserAdmin

- (NSString*) toXML {
    NSMutableString* xml = [NSMutableString string];
    [xml appendString: @"<user-admin"];
    [self writeCredentials: xml];
    if (self.operation != nil && self.operation.length > 0) {
        [xml appendFormat:@" operation=\"%@\"", self.operation];
    }
    if (self.operationUser != nil && self.operationUser.length > 0) {
        [xml appendFormat:@" operationUser=\"%@\"", self.operationUser];
    }
