...
@implementation LibreChooseCategoryViewController {
    LibreContent* selection;
    UITableView* table;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    LibreContent* category = [self.instances objectAtIndex: indexPath.row];
    cell.textLabel.text = category.name;
    UIImage* image = [self.sdk fetchImage: category.icon];
    if (image != nil) {
        image = [LibreUI resizeImage: image in: CGSizeMake(58, 58)];
        cell.imageView.image = image;
    }
    cell.detailTextLabel.text = category.contentDescription;
    return cell;
}
