...
public class Facebook extends BasicSense {
	...
	public void installApp() throws FacebookException {
		log("Installing BotLibre app on Facebook page - " + getPageId(), Level.INFO);
		Map<String, String> params = new HashMap<>();
		params.put("access_token", getFacebookMessengerAccessToken());
		params.put("subscribed_fields", "messages, messaging_postbacks");
	...
	public boolean checkAppInstalled() throws FacebookException {
		log("Checking for app installations - " + getPageId(), Level.INFO);
		...
		RawAPIResponse response = getConnection().callGetAPI(path, params);
		try {
			facebook4j.internal.org.json.JSONArray data = response.asJSONObject().getJSONArray("data");
			for (int i = 0; i < data.length(); i++) {
				facebook4j.internal.org.json.JSONObject app = data.getJSONObject(i);
				if (app.getString("name").equals(getAppName())) {
					return true;
				}
			}
			return false;
	...
	public void uninstallApp() throws FacebookException {
		log("Uninstalling BotLibre app on Facebook page - " + getPageId(), Level.INFO);
		Map<String, String> params = new HashMap<>();
		params.put("access_token", getFacebookMessengerAccessToken());
		...
		//facebook4j.Facebook con = buildConnection(getOauthKey(), getOauthSecret(), getFacebookMessengerAccessToken());
		RawAPIResponse response = getConnection().callDeleteAPI(path, params);
		saveProperties(null);
		log("App uninstall success", Level.INFO, response.asString());
	}
	...
	public void connectWebhook() throws FacebookException {
		...
		params.put("fields", "messages, messaging_postbacks");
		...
		RawAPIResponse response = getConnection().callPostAPI("/" + getOauthKey() + "/subscriptions", params);
		this.pageConnected = true;
		log("Webhook connection successful " + response.toString(), Level.INFO);
	}
	...
	public void disconnectWebhook() throws FacebookException {
		...
		log("Successfully disconnected webhook " + response.toString(), Level.INFO);
	}
	...
	public void saveProperties(List<String> autoPosts) {
		...
		memory.saveProperty("Facebook.customApp", String.valueOf(this.customApp), false);
		memory.saveProperty("Facebook.authorized", String.valueOf(this.authorized), false);
		memory.saveProperty("Facebook.pageConnected", String.valueOf(this.pageConnected), false);
	...
	public void connectPage(String pageName) throws FacebookException {
				...
				if (accounts != null) {
					for (Account account : accounts) {
						...
						if (this.page.equals(account.getName())) {
							System.out.println("***found:" + account.getName() + " :" + account.getAccessToken());
							found = true;
							this.token = account.getAccessToken();
							this.facebookMessengerAccessToken = account.getAccessToken();
		...
		/*for (Map<String, String> account : getPages()) {
			if (account.get("name").equals(pageName)) {
				...
				this.token = account.get("accessToken");
...
//			getConnection().callPostAPI("/" + id + "/messages", params);
	...
	public void input(Object input, Network network) {
		...
		try {
			if (input instanceof Post) {
				...
				inputSentence(text, userId, userName, target, post.getId(), post.getCreatedTime().getTime(), network);
				...
				if (this.processedPosts.contains(comment.getId())) {
				...
				inputSentence(text, userId, userName, this.userName, id, comment.getCreatedTime().getTime(), network);
			}
		} catch (Exception exception) {
			log(exception);
		}
	...
	public void inputSentence(String text, String userId, String userName, String targetUserName, String messageId, long time, Network network) {
		...
		Vertex user = network.createUniqueSpeaker(new Primitive(userId), Primitive.FACEBOOK, userName);
		...
		if (targetUserName != null) {
			...
			if (targetUserName.equals(getUserName())) {
				...
				targetUser = network.createUniqueSpeaker(new Primitive(targetUserName), Primitive.FACEBOOK);
			}
