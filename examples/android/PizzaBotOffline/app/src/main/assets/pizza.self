 ...
 */
state OrderPizza {
	pattern "start" template "Would you like a <button>small</button>, <button>medium</button>, or <button>large</button> pizza?";

	pattern "[stop quit exit]" template "Your order has been canceled, type 'start' to place another order?";

	pattern "*" that "Would you like a <button>small</button>, <button>medium</button>, or <button>large</button> pizza?" template setSize();

	pattern "*" that "What type of pizza would you like, <select><option></option><option>pepperoni</option> <option>hawaiian</option> <option>vegetarian</option> <option>cheese</option> </select>?" template setType();
