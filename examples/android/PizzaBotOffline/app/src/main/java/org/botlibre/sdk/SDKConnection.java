...
public class SDKConnection {
	...
	/**
	 * Create a new image attachment for an issue tracker.
	 */
	public MediaConfig createIssueTrackerImageAttachment(String file, MediaConfig config) {
		config.addCredentials(this);
		String xml = POSTIMAGE(this.url + "/create-issuetracker-attachment", file, config.name, config.toXML());
		Element root = parse(xml);
		if (root == null) {
			return null;
		}
	 ...
	 * Create a new image attachment for an issue tracker.
	 */
	public MediaConfig createIssueTrackerImageAttachment(Bitmap bitmap, MediaConfig config) {
		config.addCredentials(this);
		String xml = POSTIMAGE(this.url + "/create-issuetracker-attachment", bitmap, config.name, config.toXML());
		Element root = parse(xml);
		if (root == null) {
			return null;
		}
	 ...
	 */
	public void createGraphicMedia(String file, GraphicConfig config) {
		config.addCredentials(this);
		if ((file.indexOf(".jpg") != -1) || (file.indexOf(".jpeg") != -1)) {
			POSTIMAGE(this.url + "/update-graphic-media", file, config.fileName, config.toXML());
		} else {
			POSTFILE(this.url + "/update-graphic-media", file, config.fileName, config.toXML());
		}
	}
	/**
	...
	public String POSTIMAGE(String url, Bitmap bitmap, String name, String xml) {
		if (this.debug) {
			System.out.println("POST: " + url);
			System.out.println("XML: " + xml);
		}
