...
public class MainActivity extends LibreActivity {
	...
	public static String WEBSITE = "https://www.botlibre.com";
	...
	public static String launchInstanceName = "Pizza Bot"; // i.e. "Help Bot"
	public static String website = "https://www.botlibre.com";
	public static String prefix = "Bot";
	public static String launchSelfScriptID = "";
	...
	public static int offlineSelectedImage;
	...
	public void contentRatingList() {
		...
		contentRating = cookies.getString("contentRating", null);
		...
		builder.setMessage("Select Content Rating: ")
				.setPositiveButton("Select", new DialogInterface.OnClickListener() {
					public void onClick(DialogInterface dialog, int id) {
						// save data
						SharedPreferences.Editor editor = MainActivity.current.getPreferences(Context.MODE_PRIVATE)
								.edit();
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		launchInstanceId = cookies.getString("instanceID", launchInstanceId);
		launchInstanceName = cookies.getString("instanceName", launchInstanceName);
		// saving the a number of array as an ID, for the sake of loading the
		// exact saved bot.
		templateID = cookies.getInt("tempId", templateID);
		nameOfAvatar = cookies.getString("nameOfAvatar", nameOfAvatar);
		System.out.println("Name of avatar: " + nameOfAvatar);
		// load memory: by bot's name

		loadMemoryByBot();

		// Checking and making sure that the mic is turned off to start
		// configuring the microphone.
		micConfig = cookies.getBoolean("miconfig", micConfig);
	...
	public void loadMemoryByBot() {
		MicroMemory.storageDir = getFilesDir();
		List<OfflineTemplateConfig> items = new ArrayList<OfflineTemplateConfig>();
		items = ListTemplateView.retriveTemplates();
		OfflineTemplateConfig oneItem;
		oneItem = items.get(templateID);
		System.out.println("StorageFileName :" + oneItem.getTitle());
		offlineSelectedImage = oneItem.getImageId();
		MicroMemory.storageFileName = oneItem.getTitle();
		launchInstanceName = oneItem.getTitle();
		launchInstanceId = oneItem.getId();
	}
	...
	public boolean onOptionsItemSelected(MenuItem item) {
		...
		case R.id.menuWebsite:
			openWebsite();
			return true;
		//case R.id.menuDeleteExistingBot:
		//	MainActivity.confirm("Are you sure you want to delete your bot?", this, false,
	...
	/*
	 * This method will have a list of 8 empty bot, basic, AI, julie, eddie,
	 * alice, personal assistance, juile assistance. before attaching the ID and
	 * Name of the bot make sure that its selecetd thro the list.
	 */
	public void launch(View view) {
		...
		if (MainActivity.launchType == LaunchType.Bot) {
			if (!MicroMemory.checkExists()) {
				...
				config = new InstanceConfig();
				OfflineTemplateConfig templates = new OfflineTemplateConfig(R.drawable.bot,"Pizza Bot","",""+0,0);
				//saving a template number for getting the icons and pictures of the bot
				saveAllData(MainActivity.launchInstanceName = templates.getTitle(), MainActivity.launchInstanceId = templates.getId(), MainActivity.templateID = 0);
				...
				MainActivity.readZipAvatars(this, templates.getTitle());
				MainActivity.offlineSelectedImage = templates.getImageId();
				HttpAction action = new HttpFetchActionOffline(this, config, true);
				action.execute();
			} else {
				config = new InstanceConfig();
				readZipAvatars(this, this.nameOfAvatar);
				config.name = MainActivity.launchInstanceName;
				config.id = MainActivity.launchInstanceId;
				HttpAction action = new HttpFetchActionOffline(this, config, true);
				action.execute();
			}
		} else if (MainActivity.launchType == LaunchType.Forum) {
			config = new ForumConfig();
		} else if (MainActivity.launchType == LaunchType.Channel) {
			config = new ChannelConfig();
		}
	}
	...
	public static void changeLanguage(Activity activty, final DialogInterface.OnClickListener listener) {
		...
		ArrayAdapter adapter = new ArrayAdapter(activty, android.R.layout.simple_spinner_dropdown_item,
				MainActivity.languages);
		spin.setAdapter(adapter);
		int index = -1;
	...
	public static void readZipAvatars(Activity activity, String fileName) {
		...
		// Its only going to take one name
		try {
