...
public class ChatActivity extends LibreActivity implements TextToSpeech.OnInitListener, TextToSpeech.OnUtteranceCompletedListener, RecognitionListener {
	...
	public void onCreate(Bundle savedInstanceState) {
		...
		if (!MainActivity.online) {
			HttpGetImageAction.setImage(MainActivity.offlineSelectedImage, this.imageView);
			HttpGetImageAction.setImage(MainActivity.offlineSelectedImage, (ImageView)findViewById(R.id.responseImageView));
			HttpGetImageAction.setImage(MainActivity.offlineSelectedImage, findViewById(R.id.icon));
		} else {
			HttpGetImageAction.fetchImage(this, this.instance.avatar, findViewById(R.id.icon));
	...
	public void flagResponse() {
		MainActivity.confirm("Flag bad response?", this, true, new DialogInterface.OnClickListener() {
			@Override
			public void onClick(DialogInterface dialog, int which) {
				switch(which){
					case DialogInterface.BUTTON_POSITIVE:
						...
						config.message = "flag"; //text.getText().toString().trim();
	...
	public void changeAvatar() {
		if (MainActivity.online) {
			...
			Intent intent = new Intent(this, AvatarSelection.class);
			startActivity(intent);
		}
	}
...
//			return true;
	...
	public void MicConfiguration(){
		Intent intent = new Intent(this, MicConfiguration.class);
		startActivity(intent);
		finish();
	}
	...
	public void onBackPressed() {
		if (!MainActivity.online) {
			MainActivity.confirm("Do you want to save the current Bot's memory?", this, true,
						...
						public void onClick(DialogInterface dialog, int which) {
							...
							case DialogInterface.BUTTON_POSITIVE:
								Thread thread = new Thread(){
									@Override
									public void run(){
										Bot bot = ((MicroConnection) MainActivity.localConnection).getBot();
										bot.memory().shutdown();
									}
								};
								thread.start();
	...
	public void response(final ChatResponse response) {
		...
		try {
			...
			Log.e("ChatActivityCommand","response.command: " + response.command);
...
//						messages.add(ready);
			...
			if (stateLayouts == 3 || stateLayouts == 4) {
				Toast.makeText(ChatActivity.this, html, Toast.LENGTH_SHORT).show();
			}
