...
@javax.servlet.annotation.WebServlet("/script-meta-import")
@SuppressWarnings("serial")
@MultipartConfig
public class ScriptMetaImportServlet extends BeanServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			for (Part filePart : files) {
				...
				bean.importScript(bytes);
				count++;
			}
			if (count == 0) {
				throw new BotException("Please select the script files to import");
			}
			response.sendRedirect("script?id=" + bean.getInstanceId());
