...
@javax.servlet.annotation.WebServlet("/avatar-media-upload")
@SuppressWarnings("serial")
@MultipartConfig
public class AvatarMediaUploadServlet extends BeanServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			if (uploadURL != null && uploadURL.trim().length() > 0) {
				...
				bean.addAvatarMedia(image, fileName, fileType);
			} else {
				byte[] image = null;
				...
				for (Part filePart : files) {
					...
					bean.addAvatarMedia(image, fileName, filePart.getContentType());
					count++;
				}
				if (count == 0) {
					throw new BotException("Please select the media file to upload");
				}
			}
