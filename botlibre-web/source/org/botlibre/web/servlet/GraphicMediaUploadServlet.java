...
@javax.servlet.annotation.WebServlet("/graphic-media-upload")
@SuppressWarnings("serial")
@MultipartConfig
public class GraphicMediaUploadServlet extends BeanServlet {
	public void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			String download = (String)request.getParameter("download-graphic");
			if (download != null) {
				if (!bean.downloadMedia(response)) {
					response.sendRedirect("graphic.jsp");
				}
				return;
			}
			...
			if (uploadURL != null && uploadURL.trim().length() > 0) {
				...
				bean.saveMedia(image, fileName, fileType);
			} else {
				byte[] image = null;
				...
				for (Part filePart : files) {
					...
					bean.saveMedia(image, fileName, filePart.getContentType());
					count++;
				}
				if (count == 0) {
					throw new BotException("Please select the media file or URL to upload");
				}
			}
