...
<head>
	...
	<title>Create Graphics Link - <%= Site.NAME %></title>
	<meta name="description" content="Create a link to an external graphic website"/>	
	<meta name="keywords" content="create, link, website, graphic, image, video, audio, media"/>
...
<body>
	...
	<div id="contents">
		<div class="section">
			<h1><%= loginBean.translate("Create new graphics link") %></h1>
			<% boolean error = loginBean.getError() != null && bean.getInstance() != null; %>
			<jsp:include page="error.jsp"/>
			<% if (!loginBean.isLoggedIn()) { %>
				<p>
					<%= loginBean.translate("You must first") %> <a href="login?sign-in"><%= loginBean.translate("sign in") %></a> <%= loginBean.translate("to create a new graphics link") %>.
				</p>
			<% } else { %>
