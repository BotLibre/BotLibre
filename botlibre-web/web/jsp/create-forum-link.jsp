...
<head>
	...
	<title>Create Forum Link - <%= Site.NAME %></title>
	<meta name="description" content="Create a link to an external forum website"/>	
	<meta name="keywords" content="create, link, website, forum"/>
...
<body>
	...
	<div id="contents">
		<div class="section">
			<h1><%= loginBean.translate("Create new forum link") %></h1>
			<% boolean error = loginBean.getError() != null && bean.getInstance() != null; %>
			<jsp:include page="error.jsp"/>
			<% if (!loginBean.isLoggedIn()) { %>
				<p>
					<%= loginBean.translate("You must first") %> <a href="login?sign-in"><%= loginBean.translate("sign in") %></a> <%= loginBean.translate("to create a new forum link") %>.
				</p>
			<% } else { %>
