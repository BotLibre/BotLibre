...
<%@page import="org.botlibre.web.chat.ChatChannel"%>
...
<p />
<h3><%= loginBean.translate("Operators") %></h3>
<div class="toolbar">
	<img id="select-all-operators" src="images/select.svg" />
	<img id="remove-selected-operators" src="images/remove3.svg" />
</div>
<table id="operators-table" class="tablesorter">
	<thead>
		<tr>
			<th><%= loginBean.translate("Select") %></th>
			<th><%= loginBean.translate("Operator Name") %></th>
		</tr>
	</thead>
	<tbody>
		<% for (User user : ((ChatChannel)bean.getInstance()).getOperators()) { %>
			<tr id="<%= user.getUserId()%>">
				<td><input name="selected-operator" value="<%= user.getUserId()%>" type="checkbox" /></td>
				<td>
					<span value="<%= user.getUserId()%>"><%= user.getUserId() %></span>
				</td>
			</tr>
...
<div style="display: flex;">
	<input id="operator-input" class="users" type="text" name="newOperator" style="width: 100%;" title="<%= loginBean.translate("Enter the user id of the user to add as an operator.") %>" />
	<img id="add-new-operator" name="addOperator" src="images/plus.svg" style="margin-top: 3px;" title="<%= loginBean.translate("Add the new operator") %>" />
</div>
<script>
	$('#select-all-operators').click(function(event) {
		event.preventDefault();
		if (!selectOperators) {
			selectOperators = true;
			selectAllUsers("operators-table");
		} else {
			selectOperators = false;
			unselectAllUsers("operators-table");
		}
	});
			
	function addOperator() {
		var userName = $("input[name=newOperator]").val();
		if (userName == null || userName === "") {
			SDK.showError("Please enter operator to add.", "Add Operator Error");
			return false;
		}
		$("input[name=newOperator]").val('');
		var userAdminConfig = new UserAdminConfig();
        userAdminConfig.operation = "AddOperator";
        userAdminConfig.operationUser = userName;
		userAdminConfig.instance = "<%= bean.getInstanceId() %>";
		userAdminConfig.type = "<%=bean.getInstance().getTypeName()%>";
		sdkConnection.userAdmin(userAdminConfig, function() {
			var newRow = "<tr id='" + userName + "'><td><input name='selected-operator' value='" + userName + "' type='checkbox'></td><td><span value='" + userName + "'>" + userName + "</span></td></tr>";
			var row = $('#operators-table tbody:last-child');
			row.append(newRow);
		});
	}
			
	$('#add-new-operator').on('click', function(event) {
		event.preventDefault();
		addOperator();
	});
			
	$('#operator-input').keypress(function (event) {
		var key = event.which;
		if (key == 13) {
			addOperator();
			return false;  
	...
	$('#remove-selected-operators').click(function(event) {
		event.preventDefault();
	    var array = [];
		$('#operators-table tbody').find('tr').each(function (index, row) {
			var currRow = $(row);
		    var checkBox = currRow.find('input[type="checkbox"]');
		    if (checkBox.is(':checked')) {
		    	array.push(currRow.prop('id'));
		...
		if (array.length == 0) {
			SDK.showError("Please select operators to remove.", "Delete Operator Error");
			return false;
		}
		var sourceId = "";
		for (let i = 0; i < array.length; i++) {
		...
		var userAdminConfig = new UserAdminConfig();
        userAdminConfig.operation = "RemoveOperator";
        userAdminConfig.operationUser = sourceId;
        userAdminConfig.instance = "<%=bean.getInstanceId()%>";
        userAdminConfig.type = "<%=bean.getInstance().getTypeName()%>";
		sdkConnection.userAdmin(userAdminConfig, function() {
			for (let item in array) {
				var row = $('#operators-table tbody').find('tr#' + array[item]).remove();
		    }
		});
	});
</script>
...
<table id="users-table" class="tablesorter">
	...
	<tbody>
		...
		<tr id="<%= user.getUserId()%>">
			<td><input name="selected-user" value="<%= user.getUserId()%>" type="checkbox" /></td>
			<td>
				<span value="<%= user.getUserId()%>"><%= user.getUserId() %></span>
			</td>
		</tr>
