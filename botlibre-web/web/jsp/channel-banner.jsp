...
<div id="admin-topper" align="left" style="background-color: <%= loginBean.getBackgroundColor() %>;">
	<% if (livechatBean.getInstance() != null) { %>
		<a href=<%= "livechat?id=" + livechatBean.getInstance().getId() + proxy.proxyString() %>><%= livechatBean.getInstance().getName() %></a>
	<% } else if (proxy.getInstanceId() != null) { %>
		<a href=<%= "livechat?id=" + proxy.getInstanceId() + proxy.proxyString() %>><%= loginBean.translate("Channel") %></a>
	<% } %>
